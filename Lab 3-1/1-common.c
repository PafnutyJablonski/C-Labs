...
int cmp_pfrac(t_pfrac f1, t_pfrac f2){
    int nok = f1.c/gcd(abs(f1.c), abs(f2.c))*abs(f2.c);
    int f1b = f1.b * (nok/f1.c) + (f1.a * nok);
    int f2b = f2.b * (nok/f2.c) + (f2.a * nok);
    if (f1b < f2b) {return -1;}
    if (f1b == f2b) {return 0;}
    if (f1b > f2b) {return 1;}
}
...
int main(void){
    t_pfrac f1 = new_pfrac(0, 1, 3);
    t_pfrac f2 = new_pfrac(0, -2, 3);
    print_pfrac(pfrac_div(f1, f2));
    return 0;
};
